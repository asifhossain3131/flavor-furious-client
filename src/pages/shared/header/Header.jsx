...
const Header = () => {
   ...
   const totalFoods=cartItems?.reduce((sum,food)=>food.quantity+sum,0)
   const totalCost=cartItems?.reduce((sum,food)=>food.total+sum,0)
  ...
  <div className="dropdown dropdown-end">
      <label tabIndex={0} className="btn  btn-circle btn-ghost">
        <div className="indicator">
          ...
          <span className="badge badge-sm indicator-item">{totalFoods ||0}</span>
        </div>
      </label>
      <div tabIndex={0} className="mt-3 card card-compact dropdown-content w-52 bg-base-100 shadow">
        <div className="card-body">
          <span className="font-bold text-lg">{totalFoods}</span>
          <span className="text-info">Subtotal: ${totalCost?.toFixed(2) || 0}</span>
